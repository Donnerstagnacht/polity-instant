...
export default function EventParticipantsPage({ params }: PageParams) {
  ...
  const isAdmin = currentUserParticipation?.status === 'admin';
  ...
  const filteredProfiles = profilesData?.profiles?.filter(profile => {
    ...
    if (existingParticipantIds.includes(profile.user.id)) return false;
  ...
  const handleUpdateStatus = async (participantId: string, newStatus: 'member' | 'admin') => {
    try {
      await db.transact(
        ...
        <Dialog open={inviteDialogOpen} onOpenChange={setInviteDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Search and select users to invite to this event. They will receive an invitation to
                participate.
              </DialogDescription>
            </DialogHeader>
                ...
                <TableRow key={participant.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      {participant.user?.profile?.name || 'Unknown'}
                    </div>
                  </TableCell>
                  <TableCell>{participant.user?.profile?.contactEmail || '-'}</TableCell>
                  ...
                  <div className="flex gap-2">
                    {participant.status === 'member' && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleUpdateStatus(participant.id, 'admin')}
                      >
                    ...
                    {participant.status === 'admin' && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleUpdateStatus(participant.id, 'member')}
                      >
                        Remove Admin
                      </Button>
                    )}
