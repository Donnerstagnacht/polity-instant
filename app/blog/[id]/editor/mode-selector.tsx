...
export function ModeSelector({
  ...
  const handleModeChange = async (newMode: EditingMode) => {
    ...
    if (!isOwnerOrCollaborator) {
      toast.error('Only blog owners and collaborators can change the editing mode.');
      return;
    }
    ...
    try {
      ...
      toast.success(`Blog is now in ${newMode} mode.`);
    } catch (error) {
      console.error('Failed to change mode:', error);
      toast.error('Failed to change blog mode.');
    }
  };
