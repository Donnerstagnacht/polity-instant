...
- [x] Add `currentAgendaItemId` field to `events` entity in `db/schema/events.ts`
  - Type: `i.string().indexed().optional()` - ID of the currently active agenda item
- [x] Add `activatedAt` field to `agendaItems` entity in `db/schema/agendas.ts`
  - Type: `i.date().indexed().optional()` - Timestamp when item was activated
- [x] Add `completedAt` field to `agendaItems` entity in `db/schema/agendas.ts`
  - Type: `i.date().indexed().optional()` - Timestamp when item was completed

### 1.2 Notification Types for Agenda Activation

- [x] Add `agenda_item_activated` to `NotificationType` in `src/utils/notification-helpers.ts`

  - Add to "Event notifications" section

- [x] Create `notifyAgendaItemActivated` function in `src/utils/notification-helpers.ts`
...
- [x] Create `useAgendaNavigation` hook in `src/features/events/hooks/useAgendaNavigation.ts`
...
- [x] Add transaction logic for activating agenda item
...
- [x] Add transaction logic for completing agenda item
...
- [x] Create `AgendaNavigationControls` component in `src/features/events/ui/AgendaNavigationControls.tsx`
...
- [x] Update `EventStream.tsx` to show current agenda item prominently
...
- [x] Update `EventAgenda.tsx` to highlight active agenda item
...
- [x] Add German translations in `src/i18n/locales/de/features/events/index.ts`

  - `navigation.*` - Navigation translations

- [x] Add English translations in `src/i18n/locales/en/features/events/index.ts`
...
- [x] Add links for `eventVotingSessions`
...
- [x] Add `eventVotes` entity to `db/schema/events.ts`
...
- [x] Create `useEventVoting` hook in `src/features/events/hooks/useEventVoting.ts`
...
- [x] Create `useVotingTimer` hook in `src/features/events/hooks/useVotingTimer.ts`
...
- [x] Add `castEventVote` function to `useEventVoting` hook
...
- [x] Add vote result calculation utilities in `src/utils/voting-utils.ts`
...
- [x] Handle amendment voting result in `useEventVoting`
...
- [x] Create `handleAmendmentVoteResult` function in `src/features/amendments/utils/voting-results.ts`
...
- [x] Add change request ordering logic to voting
...
- [x] Create `useChangeRequestVoting` hook in `src/features/events/hooks/useChangeRequestVoting.ts`
...
- [x] Add CR voting result handling
  - If accepted: merge change into document (status updated to 'approved')
  - If rejected: mark CR as rejected
  - Update document content with accepted changes (handled via Plate.js editor)

### 2.7 Election Voting Flow

- [x] Create `useElectionVoting` hook in `src/features/events/hooks/useElectionVoting.ts`
...
- [x] Add election result processing in `src/utils/voting-utils.ts`
...
- [x] Handle position assignment after election
...
- [x] Add `scheduledRevoteDate` field to `positions` entity in `db/schema/groups.ts`

  - Type: `i.date().indexed().optional()`

- [x] Create `schedulePositionRevote` function in `src/features/events/utils/revote-scheduling.ts`
...
- [x] Add `scheduledElections` entity to track future elections
  ```typescript
...
- [x] Create `VotingSessionManager` component in `src/features/events/ui/VotingSessionManager.tsx`
...
- [x] Create `VotingPhaseIndicator` component in `src/features/events/ui/VotingPhaseIndicator.tsx`

  - Visual indicator for introduction/voting/completed phases
  - Timer countdown if active
  - Result display when completed

- [x] Create `VoteButtons` component in `src/features/events/ui/VoteButtons.tsx`
...
- [x] Update `EventAgendaItemDetail.tsx` to integrate voting
...
- [x] Update `AmendmentVotingQueue.tsx` to use new voting system
...
- [x] Add notification types to `NotificationType`:
...
- [x] Add `supportConfirmations` entity to `db/schema/amendments.ts`
...
- [x] Add links for `supportConfirmations`
...
- [x] Create `triggerSupporterConfirmation` function in `src/features/amendments/hooks/useSupportConfirmation.ts`
...
- [x] Update change request acceptance logic
...
- [x] Create `createConfirmationAgendaItem` function
...
- [x] Add 'support_confirmation' to agenda item types
  - Update type validation in agendaItems schema if needed

### 3.4 Version Comparison UI

- [x] Create `VersionComparisonView` component in `src/features/amendments/ui/VersionComparisonView.tsx`
...
- [x] Create `SupportConfirmationPanel` component in `src/features/amendments/ui/SupportConfirmationPanel.tsx`
...
- [x] Create `useSupportConfirmation` hook in `src/features/amendments/hooks/useSupportConfirmation.ts`
...
- [x] Update supporter display in `AmendmentWiki.tsx`
...
- [x] Verify `delete` action exists for `events` resource in RBAC

  - Already exists in `db/perms/events.ts`

- [x] Add `delete_event` to `ACTION_RIGHTS` if not present
  - `{ resource: 'events', action: 'delete', label: 'Delete/Cancel Event' }` (manage implies delete)

### 4.2 Schema Updates for Event Cancellation

- [x] Add `status` field to `events` entity in `db/schema/events.ts`

  - Type: `i.string().indexed().optional()` - 'active', 'cancelled', 'completed'
  - Default: 'active'

- [x] Add `cancellationReason` field to `events` entity

  - Type: `i.string().optional()`

- [x] Add `cancelledAt` field to `events` entity

  - Type: `i.date().indexed().optional()`

- [x] Add `cancelledBy` link to events
  - Links to the user who cancelled the event

### 4.3 Valid Reassignment Target Logic

- [x] Create `findValidReassignmentEvent` function in `src/features/events/utils/event-reassignment.ts`
...
- [x] Create `validateEventCancellation` function
...
- [x] Create `reassignAgendaItems` function in `useCancelEvent` hook
...
- [x] Handle cases where reassignment is not possible
  - Mark agenda items as 'orphaned'
  - Create notifications for collaborators

### 4.5 Amendment Path Recalculation

- [x] Create `recalculateAmendmentPath` function in `src/features/amendments/utils/path-recalculation.ts`
...
- [x] Create notification for path recalculation required
  - Type: `amendment_path_recalculation_required`
  - Include link to amendment process page

### 4.6 Event Cancellation Hook

- [x] Create `useCancelEvent` hook in `src/features/events/hooks/useCancelEvent.ts`
...
- [x] Create `CancelEventDialog` component in `src/features/events/ui/CancelEventDialog.tsx`
...
- [x] Update `EventEdit.tsx` to include cancel button

  - Only visible to users with 'delete' permission
  - Opens `CancelEventDialog`

- [x] Create `CancelledEventBanner` component
...
- [x] Create helper functions for each notification
...
- [x] Run InstantDB schema push after all schema changes
  - Verify all new entities and fields are created

### 5.2 Update Events Seeder

- [x] Update `scripts/seeders/events.seeder.ts`
...
- [x] Update `scripts/seeders/agendaAndVoting.seeder.ts`
...
- [x] Create `scripts/seeders/eventVotingSessions.seeder.ts`
...
- [x] Update `scripts/seeders/amendments.seeder.ts`
...
- [x] Create `scripts/seeders/supportConfirmations.seeder.ts`
...
- [x] Update `scripts/seeders/rbac.seeder.ts`
  - Add 'active_voting' action right to appropriate roles (already in DEFAULT_EVENT_ROLES)
  - Create 'Voter' role for events (already in constants)
  - Assign voting rights to test participants (seeded via DEFAULT_EVENT_ROLES)

### 5.8 Create Scheduled Elections Seeder

- [x] Add `scheduledElections` seeding
  - Create scheduled revotes for some positions
  - Link to groups and future events

### 5.9 Test Data Consistency

- [x] Verify all seeded data is consistent
...
| Phase                     | Tasks | Status           |
| ------------------------- | ----- | ---------------- |
| 1. Agenda Item Activation | 15    | ✅ 100% Complete |
| 2. Event Voting System    | 37    | ✅ 100% Complete |
| 3. Supporter Confirmation | 22    | ✅ 100% Complete |
| 4. Cancel Events          | 24    | ✅ 100% Complete |
| 5. Update Seed Scripts    | 17    | ✅ 100% Complete |

---

## Implementation Order

1. **Schema Updates First** - ✅ COMPLETE - All schema changes across features
2. **RBAC Updates** - ✅ COMPLETE - Added new action rights and roles
3. **Core Hooks** - ✅ COMPLETE - Implemented business logic hooks
4. **Notification Helpers** - ✅ COMPLETE - Added notification functions
5. **UI Components** - ✅ COMPLETE - All UI components integrated
6. **Translations** - ✅ COMPLETE - Added i18n support
7. **Seed Scripts** - ✅ COMPLETE - Test data generation updated
