  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      const message = selectedUsers.length === 1
        ? t('features.documents.inviteDialog.invitedOne')
        : t('features.documents.inviteDialog.invitedMany').replace('{{count}}', String(selectedUsers.length));
      ...
      toast.error(t('features.documents.inviteDialog.inviteFailed'));
    } finally {
      setIsInviting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm">
          ...
          {t('features.documents.inviteDialog.invite')}
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{t('features.documents.inviteDialog.title')}</DialogTitle>
          <DialogDescription>
            {t('features.documents.inviteDialog.description')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4">
          ...
          <Command className="rounded-lg border">
            <CommandInput
              placeholder={t('features.documents.inviteDialog.searchPlaceholder')}
              value={searchQuery}
            ...
            <CommandList>
              {isLoading ? (
                ...
                <>
                  <CommandEmpty>{t('features.documents.inviteDialog.noUsersFound')}</CommandEmpty>
                  <CommandGroup>
                        ...
                        >
                          <div className="flex flex-1 items-center gap-3">
                            ...
                            <div className="flex-1">
                              <div className="font-medium">{user.name || t('features.documents.inviteDialog.unnamedUser')}</div>
          ...
          {selectedUsers.length > 0 && (
            <div className="mt-4">
              <div className="mb-2 text-sm font-medium">{t('features.documents.inviteDialog.selected')} ({selectedUsers.length})</div>
                ...
                {selectedUsers.map(userId => {
                  ...
                  return (
                    <Badge key={userId} variant="secondary" className="gap-1 pr-1">
                      <span>{user.name || t('features.documents.inviteDialog.unnamedUser')}</span>
                      <button
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setOpen(false)} disabled={isInviting}>
            {t('features.documents.inviteDialog.cancel')}
          </Button>
          <Button onClick={handleInvite} disabled={selectedUsers.length === 0 || isInviting}>
            {isInviting ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                {t('features.documents.inviteDialog.inviting')}
              </>
            ) : (
              <>
                <UserPlus className="mr-2 h-4 w-4" />
                {t('features.documents.inviteDialog.invite')} {selectedUsers.length > 0 ? `(${selectedUsers.length})` : ''}
              </>
            )}
          </Button>
        </DialogFooter>
