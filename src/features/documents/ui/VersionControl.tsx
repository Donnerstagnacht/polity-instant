...
export function VersionControl({
  ...
  const handleCreateVersion = async () => {
    ...
    try {
      ...
      toast.success(t('features.documents.versionControl.versionCreated').replace('{{number}}', String(nextVersionNumber)));
  ...
  const handleRestoreVersion = async (version: Version) => {
    try {
      ...
      toast.success(t('features.documents.versionControl.restoredTo').replace('{{number}}', String(version.versionNumber)));
  ...
  const getCreationTypeBadge = (type: string) => {
    ...
    > = {
      ...
      suggestion_added: { label: t('features.documents.versionControl.types.suggestionAdded'), variant: 'secondary' },
      suggestion_accepted: { label: t('features.documents.versionControl.types.accepted'), variant: 'outline' },
      suggestion_declined: { label: t('features.documents.versionControl.types.declined'), variant: 'destructive' },
    };
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="version-title">{t('features.documents.versionControl.versionTitle')}</Label>
              <Input
                id="version-title"
          ...
          <DialogFooter>
            <Button onClick={handleCreateVersion} disabled={isCreating || !versionTitle.trim()}>
              {isCreating ? t('features.documents.versionControl.creating') : t('features.documents.versionControl.createVersion')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          <ScrollArea className="h-[400px] pr-4">
            {isLoading ? (
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {version.creator?.name || version.creator?.email || t('features.documents.versionControl.unknown')}
                        </div>
                      </div>
                    </div>
                    <Button
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-muted-foreground">{t('features.documents.versionControl.noVersionsYet')}</p>
