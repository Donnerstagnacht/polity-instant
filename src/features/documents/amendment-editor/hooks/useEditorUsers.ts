 ...
 *
 * @deprecated Use `useEditorUsers` from `@/features/editor` instead.
 * Import: `import { useEditorUsers } from '@/features/editor';`
 * Usage: `const editorUsers = useEditorUsers(entity, currentUser);`
 */
...
export function useEditorUsers(
  ...
  currentUserRecord: User | undefined,
    ...
    if (currentUser && currentUserRecord) {
      users[currentUser.id] = {
        ...
        avatarUrl:
          ...
          `https://api.dicebear.com/9.x/glass/svg?seed=${currentUser.id}`,
      };
    }
