...
export function UserWiki() {
  ...
  const getStatusStyles = (status: UserAmendment['status']) => {
    switch (status) {
      ...
      case 'Rejected':
        return {
          badge: 'destructive', // This stays as destructive for the background
          bgColor: 'bg-red-50 dark:bg-red-900/20',
          textColor: 'text-red-800 dark:text-red-300',
          badgeTextColor: 'text-white', // Adding specific text color for the badge
        };
      case 'Under Review':
        return {
          badge: 'secondary',
  ...
  return (
    <div className="container mx-auto max-w-6xl p-4">
      ...
      {/* Stats section - now without card wrapping */}
      <div className="mb-6 flex flex-wrap justify-between">
        {user.stats.map((stat, index) => (
          <div key={index} className="min-w-[80px] flex-1 px-2 py-2 text-center">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-gray-800 hover:text-gray-600 dark:text-gray-300 dark:hover:text-gray-400"
                >
                  <Twitter size={24} />
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                {filteredBlogs.map(blog => {
                  ...
                  return (
                    <Card
                      key={blog.id}
                      className={`overflow-hidden ${gradientClass} flex h-full flex-col`}
                    >
