 ...
 * EXCEPT those belonging to a specific suggestion ID.
 *
 * Used for the "single suggestion" editor view:
 * the document shows the current state of the text with only one chosen
 * suggestion highlighted, so the user can focus on that specific change.
 */
...
/**
 * Filter a plate-js document to only show a single suggestion.
 *
 ...
 * The target suggestion is left intact with its marks.
 *
 * @param content - The plate-js document JSON
 * @param targetSuggestionId - The suggestion ID to keep
 * @returns A new document Value with only the target suggestion's marks
 */
export function filterDocumentToSingleSuggestion(
  content: Value,
  targetSuggestionId: string,
): Value {
  return processNodes(content, targetSuggestionId) as Value;
}
...
function processNodes(
  ...
  for (const node of nodes) {
    const processed = processNode(node, targetId);
...
function processNode(
  ...
  if (allSuggestionKeys.length === 0) {
    ...
    if ('children' in node && Array.isArray(node.children)) {
      const processedChildren = processNodes(node.children, targetId);
      return [{ ...node, children: processedChildren }];
    }
    return [node];
  }

  // Check if this node has the target suggestion
