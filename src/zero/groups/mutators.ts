...
export const groupMutators = {
  ...
  update: defineMutator(groupUpdateSchema, async ({ tx, args }) => {
    await tx.mutate.group.update({ ...args, updated_at: Date.now() })
  }),

  delete: defineMutator(groupDeleteSchema, async ({ tx, args }) => {
  ...
  inviteMember: defineMutator(groupMembershipCreateSchema, async ({ tx, args }) => {
    const now = Date.now()
    await tx.mutate.group_membership.insert({
      ...args,
  ...
  acceptInvitation: defineMutator(
  ...
  createPositionHolderHistory: defineMutator(createPositionHolderHistorySchema, async ({ tx, args }) => {
    const now = Date.now()
    await tx.mutate.position_holder_history.insert({ ...args, created_at: now })
  }),

  updatePositionHolderHistory: defineMutator(updatePositionHolderHistorySchema, async ({ tx, args }) => {
