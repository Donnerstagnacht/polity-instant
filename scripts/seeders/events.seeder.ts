  ...
  async seed(context: SeedContext): Promise<SeedContext> {
    ...
    for (const config of recurringConfigs) {
      ...
      const parentEventTx = tx.events[parentEventId].update({
        ...
        ...(config.pattern === 'monthly' || config.pattern === 'yearly' || config.pattern === 'four-yearly' ? {
      ...
      const instances = generateRecurringInstances({ startDate }, config.pattern, config.occurrences - 1);
      ...
      for (const instance of instances) {
        ...
        const instanceTx = tx.events[instance.instanceId].update({
          ...
          ...(config.pattern === 'monthly' || config.pattern === 'yearly' || config.pattern === 'four-yearly' ? {
            delegateNominationDeadline: new Date(instance.date.getTime() - 14 * 24 * 60 * 60 * 1000),
            proposalSubmissionDeadline: new Date(instance.date.getTime() - 7 * 24 * 60 * 60 * 1000),
    ...
    for (let i = 0; i < numEvents; i++) {
      ...
      const eventTx = tx.events[eventId].update({
        ...
        tags: randomItems(['meetup', 'workshop', 'conference', 'social', 'training'], randomInt(1, 3)),
        createdAt: faker.date.past({ years: 0.5 }),
        updatedAt: faker.date.recent({ days: 7 }),
