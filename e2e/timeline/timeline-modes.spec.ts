...
test.describe('Timeline - Mode Switching', () => {
  ...
  test('Mode preference persists across page navigation', async ({ authenticatedPage: page }) => {
    ...
    // Navigate away and back (dismiss dialog again on return to /)
  ...
  test('Following mode shows content from subscribed entities', async ({ authenticatedPage: page }) => {
    ...
    const timelineCards = page.locator('main a[href*="/group/"], main a[href*="/event/"], main a[href*="/amendment/"], main a[href*="/blog/"]');
    const emptyState = page.getByText(/subscribe|discover|no content|nothing to show/i);

    // Wait for either content cards or empty state to appear
    await timelineCards.first().or(emptyState).waitFor({ state: 'visible', timeout: 15000 }).catch(() => {});
...
test.describe('Timeline - Mode Visual Indicators', () => {
  ...
  test('Mode toggle is visible and accessible', async ({ authenticatedPage: page }) => {
    // Wait for the mode toggle tabs to appear
    ...
    const tabCount = await modeButtons.count();
